...
export default {
  ...
  getAnnotatedFiles(file_path) {
      ...
      var dir_path  = self.getParentDirectory(file_path)
  ...
  setListnersonParent(parent, children){
    var updatedaParent = self.modifyPath(parent)
    console.log("Updated Parent " + updatedaParent)
    console.log("Children " + children)
    var query = self.getSpanElementsforDataPath(updatedaParent)
    ...
    try{
      ...
      var newParent = self.getParentDirectory(parent)
            ...
            try {
              ...
              for (j = 0, len1 = ref.length; j < len1; j++) {
                ...
                if (entry.annotations.length > 0) {
                  ...
                  var updatedsfile = self.convertUnixPathtoWindows(sfile)
                  console.log("Updated SFile" + updatedsfile)
                  var dir = self.getParentDirectory(updatedsfile)
  ...
  getAnnotatedFilesUnderaDirectory(dirpath){
    ...
    self.searchAnnoatedFilesUnderFolder(dirpath).then(function(filesList) {
      ...
      var updatedParent = self.modifyPath(dirpath)
      var superParent = self.getParentDirectory(dirpath)
      var updatedsuperParent = self.modifyPath(superParent)
      console.log("Super Parent " + updatedsuperParent)
      if (filesList.length > 0) {
        for(i=0;i<filesList.length;i++){
          var updatedFile = self.modifyPath(filesList[i])
          var curr = updatedFile
            do{
              console.log("Updated Parent " + updatedFile)
              var revFile1 = self.demodifyPath(curr)
              var query = self.getSpanElementsforDataPath(curr)
              if(query.length > 0){
                self.setColoronelement(query[0])
                query[0].addEventListener('click', function(){
              ...
              var revFile = self.demodifyPath(curr)
              var tempfile = self.getParentDirectory(revFile)
              curr = self.modifyPath(tempfile)
