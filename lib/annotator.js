...
export default {
  ...
  activate(state) {
    ...
    var rootPath    = directories[0].getPath()
  ...
  applyCSSStyling(file_path) {
    var updatedaPath = file_path.replace(/\\/g , '\\\\')
    console.log("Updated Path " + updatedaPath )

    var fileHasAnnotatations = self.getAnnotatedFiles(file_path)
    //var newquery = document.querySelectorAll("span[data-path='C:\\\\Users\\\\Gopala\\\\Documents\\\\GitHub\\\\cc-annotator\\\\Annotator-Plugin\\\\lib\\\\handler\\\\.annotator']")
    console.log("File has Annottations? : " + fileHasAnnotatations)
    if(fileHasAnnotatations){
      //var newquery = document.querySelectorAll("span[data-path='" + updatedaPath + "']")
      //console.log("New query " + newquery[0])
      var children = new Array()
      self.setListnersonParent(file_path, children)
    }
  },
  getAnnotatedFiles(file_path) {
      var fileHasAnnotatations = false
      var dir_path  = file_path.substring(0, file_path.lastIndexOf(path_lib.sep))
  ...
  setListnersonParent(parent, children){
    var updatedaParent = parent.replace(/\\/g , '\\\\')
    console.log("Updated Parent " + updatedaParent)
    console.log("Children " + children)
    var query = document.querySelectorAll("span[data-path='" + updatedaParent + "']")
    console.log("Parent length " + query.length)
    try{
      query[0].style.color = 'blue'
      query[0].addEventListener('click', function(){
        self.getAnnotatedFilesUnderaDirectory(parent)
      });
      /*query[0].addEventListener('click',  setTimeout(function(){
        atom.notifications.addInfo("Click event detected")
        console.log("Clicked Children " + children)
        for (i = 0; i < children.length; i++){
          var updatedChild = children[i].replace(/\\/g , '\\\\')
          console.log("Updated Child " + updatedChild)
          var childquery = document.querySelectorAll("span[data-path='" + updatedChild + "']")
          console.log("Child Query Length " + childquery.length)
          console.log(childquery[0])
          if(childquery.length > 0){
            self.setColoronelement(childquery[0])
          }
        }
      }), 3000);*/
    }catch(err){
      ...
      var newParent = parent.substring(0, parent.lastIndexOf(path_lib.sep))
      children[children.length] = parent
      if(newParent != ""){
        self.setListnersonParent(newParent,children)
      }
    }
  },
            ...
            try {
              ...
              for (j = 0, len1 = ref.length; j < len1; j++) {
                ...
                if (entry.annotations.length > 0) {
                  ...
                  var updatedsfile = sfile.replace(/\//g , '\\')
                  console.log("Updated SFile" + updatedsfile)
                  var dir = updatedsfile.substring(0, updatedsfile.lastIndexOf(path_lib.sep))
  ...
  getAnnotatedFilesUnderaDirectory(dirpath){
    ...
    self.searchAnnoatedFilesUnderFolder(dirpath).then(function(filesList) {
      console.log("FileList " + filesList)
      var updatedParent = dirpath.replace(/\\/g , '\\\\')
      var superParent = dirpath.substring(0, dirpath.lastIndexOf(path_lib.sep))
      var updatedsuperParent = superParent.replace(/\\/g , '\\\\')
      console.log("Super Parent " + updatedsuperParent)
      if (filesList.length > 0) {
        for(i=0;i<filesList.length;i++){
          var updatedFile = filesList[i].replace(/\\/g , '\\\\')
          var curr = updatedFile
            do{
              console.log("Updated Parent " + updatedFile)
              var revFile1 = curr.replace(/\\\\/g , '\\')
              var query = document.querySelectorAll("span[data-path='" + curr + "']")
              if(query.length > 0){
                self.setColoronelement(query[0])
                query[0].addEventListener('click', function(){
                  self.getAnnotatedFilesUnderaDirectory(revFile1)
                });
              }
              var revFile = curr.replace(/\\\\/g , '\\')
              var tempfile = revFile.substring(0, revFile.lastIndexOf(path_lib.sep))
              curr = tempfile.replace(/\\/g , '\\\\')
              console.log("New Current " + curr)
              console.log("Current Parent " + updatedParent)
              console.log("Equality " + (curr == updatedParent))
