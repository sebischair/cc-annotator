...
configUri = 'atom://annotator_configuration';
filedashUri = 'atom://filedashboard';
...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.workspace.addOpener((function(_this) {
          return function(filePath, dir) {
            if (filePath === configUri) {
              return _this.createConfigView({
                uri: dashUri
              });
            }
          };
     ...
     this.patternsModal = atom.workspace.addModalPanel({
      ...
      this.subscriptions.add(atom.workspace.addOpener((function(_this) {
          return function(filePath, dir) {
            ...
            else if(filePath === filedashUri){
              return _this.createFileDashboardView({
                uri: dashUri,
                path: dir
              });
            }
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'annotator:open_configuration': () => this.open_configuration()
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'annotator:open_filedashboard': () => this.open_filedashboard()
    }));
  ...
  createConfigView: function(state) {
    var configView;
    configView = require('./handler/config_view');
    return new configView(state);
  },
  createFileDashboardView: function(state) {
    var FileDashboardView;
    FileDashboardView = require('./handler/filedashboard');
    return new FileDashboardView(state);
  },
  ...
  setColoronelement(element){
    ...
    element.setAttribute('isAnnotated', 'true');
    //element.classList.add('.icon-file-text')
    //var mix = less.functions.functionRegistry.get('.add-icon-to-folder');
    //console.log("Mix " + mix)
    //console.log(mix('Test1'));
  },
  ...
  get_custom_annotations(){
    ...
    query.sebis_services_get_with_parameter(url_sentiment, qs).then((response) => {
          ...
          for (var i = 0; i < smells.length; i++){
            var smell = smells[i]
            var re = new RegExp("/","g");
            smell.path = smell.path.replace(re, path_lib.sep)
  ...
  */
  ...
  open_filedashboard(){
    ...
    editor = atom.workspace.getActiveTextEditor()
    filepath = editor.getPath()
    atom.workspace.open(filedashUri, filepath)
  },
