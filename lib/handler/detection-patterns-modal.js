...
module.exports =  class DetectionPatternModal {
  ...
  createFiledRow(pattern){
        ...
        this.single_value.setAttribute("mini", "");
        this.single_value.setAttribute('id', id_ctr);
        this.single_value.getModel().setText(pattern.tags || "");
        this.pattern_container.appendChild(this.single_value);
  ...
  submit_patterns(){
    console.log(self.passed_data)
    containers = self.passed_data.getElementsByClassName('pattern_submit_name');

    new_patterns = []
    for (i = 1; i < containers.length; i++){

        name =
          self.passed_data.getElementsByClassName('pattern_submit_name')[i].getModel().getText();
        regex =
          self.passed_data.getElementsByClassName('pattern_submit_pattern')[i].getModel().getText();
        descr =
          self.passed_data.getElementsByClassName('pattern_submit_descr')[i].getModel().getText();
        tags  =
          self.passed_data.getElementsByClassName('pattern_submit_tags')[i].getModel().getText();
        lang  =
          self.passed_data.getElementsByClassName('pattern_submit_lang')[i].getModel().getText();
    ...
    //self.destroy()
    self.annotator.patternsModal.hide();

    submission = {
      projectId: patterns.projectId,
