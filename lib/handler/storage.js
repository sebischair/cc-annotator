...
module.exports =  {
  ...
  store_json: function(path, json) {
    fs.writeFile(path, JSON.stringify(json, undefined, 4))
  },
  ...
  */
  ...
  store_custom_annotator_file: function(file_to_store, result){
    var base_path = file_to_store.substring(0, file_to_store.lastIndexOf(path_lib.sep)+1)
    var annotator_file = base_path + ".annotator"

    if (this.isFile(annotator_file)){
      var content = this.load_json(annotator_file)
      content = this.update_custom_content(content, result)
      console.log(content)
      this.store_json(annotator_file, content)
    } else {
      var content = this.generate_annotator_content(result)
  ...
  update_content(content, result){
    ...
    for (var i = 0; i < annotated_files.length; i++){
      ...
      if (file.meta.name === result.meta.name){
        annotated_files[i].annotations = result.annotations
        updated = true
      }
    }
  ...
  update_custom_content(content, result){
    // TODO Update content
    var annotated_files = content.annotated_files
    var updated = false
    for (var i = 0; i < annotated_files.length; i++){
      var file = annotated_files[i]
      if (file.meta.name === result.meta.name){
        file.custom_annotations = result.custom_annotations
        updated = true
      }
    }
  ...
  get_root_dir_for_file(file_name){
      var rootDirectories = atom.project.rootDirectories

      for (var i = 0; i < rootDirectories.length; i++){
          var root_dir = rootDirectories[i].path
  ...
  get_relative_file_name(file_name){
        var rootDirectories = atom.project.rootDirectories

        for (var i = 0; i < rootDirectories.length; i++){
            if(file_name.startsWith(rootDirectories[i].path)){
                return file_name.replace(rootDirectories[i].path, "")
            }
        }
